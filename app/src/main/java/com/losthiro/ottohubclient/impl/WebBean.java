...
public class WebBean {
    ...
    public void loadHTML(WebView view) {
        ...
        StringBuilder htmlBulider=new StringBuilder("<!DOCTYPE html><html><head><title>Loading...</title><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">");
...
//            text = text.replace("&#39;", "'");
    ...
    private void setScripts(StringBuilder html) {
        for (String js: scripts) {
            html.append("<script src=\"").append(js).append("\"/>");
        }
    }
    ...
    private String replaceAt(String htmlContent) {
        ...
        while (matcher.find()) {
            ...
            matcher.appendReplacement(sb, "<span class=\"colored-text\" style=\"color:" + getColor("e") + "\">" + match + "</span>");
        }
    ...
    private String replaceLinks(String htmlContent) {
        ...
        while (matcher.find()) {
            ...
            matcher.appendReplacement(sb, "<a href=\"https://m.ottohub.cn/" + getType(type) + num + "\">" + match + "</a>");
        }
        matcher.appendTail(sb);
        return sb.toString();
    }
