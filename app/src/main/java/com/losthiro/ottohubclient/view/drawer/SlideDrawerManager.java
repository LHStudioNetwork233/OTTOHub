...
public class SlideDrawerManager {
	...
	public void registerDrawer(View parent, final ImageButton slideButton) {
		if (parent == null) {
			return;
		}
	...
	public void showAccountSwitch(View v) {
		AccountSwitchWindow.getInstance(v.getContext(), action).switchShow(v);
	}

	public void setOnAccountChangeListener(Context c, AccountManager.AccountListener listener) {
		AccountSwitchWindow.getInstance(c, action).setOnAccountChangeListener(listener);
	}

	public static interface UpdateDrawer {
		void update(Account now);
	}
}
