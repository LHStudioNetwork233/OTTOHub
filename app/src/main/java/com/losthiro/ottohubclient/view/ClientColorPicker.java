...
public class ClientColorPicker extends View {
	...
	public ClientColorPicker(Context context) {
		this(context, null);
	}

	public ClientColorPicker(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public ClientColorPicker(Context context, AttributeSet attrs, int defStyle) {
	...
	private void drawSatValPanel(Canvas canvas) {
		...
		canvas.drawRect(mDrawingRect.left, mDrawingRect.top, mSatValRect.right + BORDER_WIDTH,
				mSatValRect.bottom + BORDER_WIDTH, mBorderPaint);
	...
	private void drawHuePanel(Canvas canvas) {
		...
		mBorderPaint.setColor(mBorderColor);
		canvas.drawRect(rect.left - BORDER_WIDTH, rect.top - BORDER_WIDTH, rect.right + BORDER_WIDTH,
				rect.bottom + BORDER_WIDTH, mBorderPaint);
		//初始化H线性着色器
		if (mHueShader == null) {
			int[] hue = new int[361];
			int count = 0;
			...
			mHueShader = new LinearGradient(rect.left, rect.top, rect.left, rect.bottom, hue, null, LinearGradient.TileMode.CLAMP);
